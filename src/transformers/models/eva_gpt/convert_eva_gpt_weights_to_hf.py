...
def write_model(
    ...
    if "rope_parameters" in original_config:
        ...
        # Old format: construct rope_parameters from individual keys with defaults matching EvaGptConfig
        rope_parameters = {
    ...
    if not mxfp4:
        print("Loading the checkpoint in a EvaGpt model for unpacked format")
        with torch.device("meta"):
            model = EvaGptForCausalLM(config)
        ...
        config.architectures = ["EvaGptForCausalLM"]
    ...
    EvaGptForCausalLM.from_pretrained(model_path, dtype=torch.bfloat16, device_map="auto")
...
class EvaGptConverter(TikTokenConverter):
