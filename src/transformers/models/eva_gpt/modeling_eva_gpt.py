#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# 									NOTE:
# 					This file is based on the GPT-OSS architecture and adapted for EvaGPT.
# 					Manual modifications have been applied to integrate EvaGptConfig and naming.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# coding=utf-8
...
class EvaGptRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        EvaGptRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class GptOssMLP(nn.Module):
    def __init__(self, config):
        ...
        self.experts = EvaGptExperts(config)
...
class GptOssRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: EvaGptConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: Optional[EvaGptConfig] = None,
...
class EvaGptAttention(nn.Module):
    ...
    def __init__(self, config: EvaGptConfig, layer_idx: int):
        super().__init__()
...
class EvaGptDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: EvaGptConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = EvaGptAttention(config=config, layer_idx=layer_idx)
        self.mlp = GptOssMLP(config)
        self.input_layernorm = EvaGptRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = EvaGptRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class EvaGptPreTrainedModel(PreTrainedModel):
    config: EvaGptConfig
...
@auto_docstring
class EvaGptModel(EvaGptPreTrainedModel):
    _no_split_modules = ["EvaGptDecoderLayer"]

    def __init__(self, config: EvaGptConfig):
        ...
        self.layers = nn.ModuleList(
            [EvaGptDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = EvaGptRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class EvaGptForCausalLM(EvaGptPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, EvaGptForCausalLM

        >>> model = EvaGptForCausalLM.from_pretrained("MTSmash/EvaGPT-German-v9-1-2-1")
        >>> tokenizer = AutoTokenizer.from_pretrained("MTSmash/EvaGPT-German-v9-1-2-1")
...
class GptOssForSequenceClassification(GenericForSequenceClassification, EvaGptPreTrainedModel):
    pass


class GptOssForTokenClassification(GenericForTokenClassification, EvaGptPreTrainedModel):
    pass
