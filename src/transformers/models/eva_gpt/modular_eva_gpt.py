...
class EvaGptRMSNorm(LlamaRMSNorm):
...
class EvaGptTopKRouter(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class EvaGptMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.router = EvaGptTopKRouter(config)
...
class EvaGptRotaryEmbedding(Qwen2RotaryEmbedding):
    @torch.no_grad()
...
class EvaGptAttention(Qwen2Attention):
...
class EvaGptDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: EvaGptConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.hidden_size = config.hidden_size
        self.self_attn = EvaGptAttention(config=config, layer_idx=layer_idx)
        self.mlp = EvaGptMLP(config)
...
class EvaGptPreTrainedModel(LlamaPreTrainedModel):
    _keep_in_fp32_modules = ["post_attention_layernorm", "input_layernorm", "norm"]
    _supports_sdpa = True
    _supports_flash_attention = False
    _supports_flex_attention = False
    _can_record_outputs = {
        "router_logits": OutputRecorder(EvaGptTopKRouter, index=0),
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, EvaGptTopKRouter):
            init.normal_(module.weight, mean=0.0, std=std)
            init.normal_(module.bias, mean=0.0, std=std)


class EvaGptModel(MixtralModel):
...
class EvaGptForCausalLM(MixtralForCausalLM):
    pass


class EvaGptForSequenceClassification(MixtralForSequenceClassification):
    pass


class EvaGptForTokenClassification(MixtralForTokenClassification):
    pass


__all__ = [
    "EvaGptForCausalLM",
    "EvaGptForSequenceClassification",
    "EvaGptForTokenClassification",
    "EvaGptModel",
